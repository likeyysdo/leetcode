...
class Solution {
    public int minArray(int[] numbers) {
        ...
        int start = 0;
        int end = length - 1;
        return a(numbers,start,end);
    }

    public int a( int[] numbers , int start , int end ){
        if( start == end ){
        ...
        int mid = (start + end) /2 ;
        int m = numbers[mid];
        return Math.min(a(numbers,start,mid),a(numbers,mid+1,end));
    }
}
